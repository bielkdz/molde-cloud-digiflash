...
import {
 ...
 listOneDriveFolders,
 restoreOneDriveAccount,
 uploadPhotoToOneDrive,
} from "./onedrive";

type Screen="dashboard"|"capture"|"files"|"history"|"search";
...
function DashboardApp({user,notice}:{user:User;notice:string}){
 const [screen,setScreen]=useState<Screen>("dashboard"),[collapsed,setCollapsed]=useState(true),[connected,setConnected]=useState(false),[connecting,setConnecting]=useState(false);
 const [folders,setFolders]=useState<string[]>([]),[folder,setFolder]=useState(""),[newFolder,setNewFolder]=useState(""),[showFolder,setShowFolder]=useState(false);
 const [fileName,setFileName]=useState(""),[photo,setPhoto]=useState<string|null>(null),[photoFile,setPhotoFile]=useState<File|null>(null),[query,setQuery]=useState(""),[driveNotice,setDriveNotice]=useState(""),[uploading,setUploading]=useState(false); const inputRef=useRef<HTMLInputElement>(null);
 const filtered=useMemo(()=>recent.filter(x=>`${x.name} ${x.folder}`.toLowerCase().includes(query.toLowerCase())),[query]);
 useEffect(()=>{if(!isOneDriveConfigured)return;void restoreOneDriveAccount().then(async account=>{if(!account)return;setConnected(true);const items=await listOneDriveFolders();setFolders(items.map(x=>x.name));if(items[0])setFolder(items[0].name)}).catch(()=>setDriveNotice("Não foi possível restaurar a conexão com o OneDrive."))},[]);
 function choosePhoto(file?:File){if(!file)return;setPhotoFile(file);setPhoto(URL.createObjectURL(file));if(!fileName)setFileName(file.name.replace(/\.[^/.]+$/, ""));}
 async function handleConnect(){setDriveNotice("");if(!isOneDriveConfigured){setDriveNotice("A integração Microsoft está preparada, mas ainda falta registrar o aplicativo OneDrive.");return}setConnecting(true);try{await connectOneDrive();const items=await listOneDriveFolders();setFolders(items.map(x=>x.name));if(items[0])setFolder(items[0].name);setConnected(true);setDriveNotice("OneDrive conectado com sucesso.")}catch{setDriveNotice("Não foi possível conectar ao OneDrive. Tente novamente.")}finally{setConnecting(false);setCollapsed(true)}}
 async function createFolder(){const v=newFolder.trim();if(!v||folders.includes(v))return;if(!connected){setDriveNotice("Conecte o OneDrive antes de criar uma pasta.");return}try{const created=await createOneDriveFolder(v);setFolders([...folders,created.name]);setFolder(created.name);setNewFolder("");setShowFolder(false);setDriveNotice(`Pasta ${created.name} criada no OneDrive.`)}catch{setDriveNotice("Não foi possível criar a pasta no OneDrive.")}}
 async function uploadPhoto(){if(!photoFile||!fileName||!folder)return;setUploading(true);setDriveNotice("");try{await uploadPhotoToOneDrive(folder,fileName,photoFile);setPhoto(null);setPhotoFile(null);setFileName("");if(inputRef.current)inputRef.current.value="";setDriveNotice("Foto enviada para o OneDrive com sucesso.");setScreen("history")}catch{setDriveNotice("Não foi possível enviar a foto para o OneDrive.")}finally{setUploading(false)}}
 ...
 return <main className={`app-shell ${collapsed?"is-collapsed":""}`}>
  ...
  <aside className="sidebar"><div className="brand"><span className="brand-mark">M</span><span className="brand-copy">Molde Cloud<small>DIGIFLASH</small></span></div><button className="collapse" onClick={()=>setCollapsed(!collapsed)} aria-label="Recolher menu">‹</button><nav>{nav.map(x=><button key={x.id} className={screen===x.id?"active":""} onClick={()=>navigateTo(x.id)}><span className="nav-icon"><Icon name={x.icon}/></span><span className="nav-label">{x.label}</span></button>)}</nav><button className={`onedrive-button ${connected?"connected":""}`} disabled={connecting} onClick={handleConnect}><span className="nav-icon"><Icon name="cloud"/></span><span className="nav-label">{connecting?"Conectando...":connected?"OneDrive conectado":"Conectar OneDrive"}</span></button></aside>
  <section className="workspace"><header><div><p>DIGIFLASH</p><h1>{nav.find(x=>x.id===screen)?.label}</h1></div><div className="header-actions"><div className={`status ${connected?"ok":""}`}><i/>{connected?"OneDrive conectado":"OneDrive desconectado"}</div><button className="user-chip" onClick={()=>signOut(auth)} title="Sair da conta">{user.photoURL?<img src={user.photoURL} alt=""/>:<span>{user.displayName?.[0]||"U"}</span>}<b>{user.displayName?.split(" ")[0]||"Usuário"}</b></button></div></header>{(notice||driveNotice)&&<div className="system-notice">{driveNotice||notice}</div>}
   {screen==="dashboard"&&<Dashboard connected={connected} go={setScreen}/>} 
   {screen==="capture"&&<Capture photo={photo} folder={folder} folders={folders} fileName={fileName} setFolder={setFolder} setFileName={setFileName} inputRef={inputRef} openFolder={()=>setShowFolder(true)} connected={connected} uploading={uploading} upload={uploadPhoto}/>}
